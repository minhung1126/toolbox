      ...
      <div className="page-actions settings-card-actions">
        {showConnections && <>
          ...
          {routingMode === YOUTUBE_ROUTING_MODES.MANUAL && record.can_be_active && !isActive && (
            <button className="btn btn-secondary" onClick={() => activateSlot(slot)} type="button" disabled={pageBusy || authorizationBusy}>
              設為作用中
            </button>
          )}
          {record.authenticated && (
            <button className="btn" onClick={() => requestDisconnect(slot)} type="button" disabled={pageBusy || authorizationBusy}>
              斷開
            </button>
          )}
        </>}
