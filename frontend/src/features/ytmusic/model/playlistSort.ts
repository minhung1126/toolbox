import type { PlaylistSortKey, PlaylistSortTrack, PlaylistSortPreview } from './types';

/**
 * Pure sorting, normalization and collation algorithms for YouTube Music playlists.
 */

export function getLocaleCollation(lang: unknown) {
...
export const SORT_PRESETS: { id: string; label: string; keys: PlaylistSortKey[] }[] = [
  {
    id: 'album-order',
...
export function normalizeArtistName(name: unknown) {
  if (!name) return '';
...
export function splitArtists(name: unknown) {
  if (!name) return [];
  const raw = String(name).trim();
  if (!raw) return [];
  ...
  if (featMatch) {
    ...
    cleaned = (cleaned.slice(0, featMatch.index) + cleaned.slice((featMatch.index ?? 0) + featMatch[0].length)).trim();
...
export function getFirstArtist(name: unknown) {
...
export type AlbumContextMap = Record<string, AlbumContext>;

export function buildAlbumContextMap(items: readonly PlaylistSortTrack[]): AlbumContextMap {
  const albumTracks: Record<string, PlaylistSortTrack[]> = Object.create(null);
  ...
  const contextMap: AlbumContextMap = Object.create(null);
  for (const [normAlbum, tracks] of Object.entries(albumTracks)) {
    let explicitAlbumArtist: string | null = null;
    ...
    const artistCounts: Record<string, number> = Object.create(null);
    for (const trk of tracks) {
    ...
    let dominantArtist: string | null = null;
    let maxCount = 0;
    ...
    let albumYear: string | number | null = null;
...
export function getEffectiveSortArtist(item: PlaylistSortTrack, albumMap: AlbumContextMap = {}) {
...
export function sortTracksLocally(
  items: readonly PlaylistSortTrack[] | null | undefined,
  sortKeys: readonly PlaylistSortKey[] | null | undefined,
  locale = 'zh-Hant-TW'
): PlaylistSortTrack[] {
  ...
  for (let k = sortKeys.length - 1; k >= 0; k--) {
    ...
    sorted.sort((a, b) => {
      ...
      if (field === 'album') {
        ...
        const aVal = aIsReal ? String(a.album).normalize('NFKC').toLowerCase() : '';
        const bVal = bIsReal ? String(b.album).normalize('NFKC').toLowerCase() : '';
...
export function buildPreviewFromSorted(
  originalItems: readonly PlaylistSortTrack[],
  sortedItems: readonly PlaylistSortTrack[]
): PlaylistSortPreview {
  const origPositions: Record<string, number> = Object.create(null);
