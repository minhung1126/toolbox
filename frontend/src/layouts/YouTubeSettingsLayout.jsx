...
import '../features/youtube/youtube-settings.css';
...
export default function YouTubeSettingsLayout() {
  ...
  return (
    <div className="section-gap settings-page">
      ...
      <nav className="youtube-settings-subnav" aria-label="YouTube 設定子導覽">
        {items.map(([to, label]) => {
          ...
          return (
            <NavLink
              ...
              to={to}
              className={({ isActive }) =>
                `youtube-settings-subnav-link${isActive || isCurrentTabActive ? ' active' : ''}`
              }
            >
              {label}
            </NavLink>
